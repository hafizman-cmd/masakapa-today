...
export default function RecipeCard({ recipe, match, isFavorite = false, onToggleFavorite, onClick, lang = 'ms' }) {
  const name = text(recipe.name, lang); const initials = name.split(' ').slice(0, 2).map(word => word[0]).join('')
  const optionalNote = match?.missingOptional?.length ? `${lang === 'ms' ? 'Tiada' : 'Missing'}: ${match.missingOptional.map(item => text(item.name, lang)).join(', ')}` : ''
  ...
  return <div className="flex items-center justify-between p-3 bg-white rounded-xl border border-stone-100 shadow-sm gap-3 hover:bg-stone-50 transition-colors text-left w-full" role="button" tabIndex="0" onClick={() => onClick(recipe, match?.missingCore)} onKeyDown={event => event.key === 'Enter' && onClick(recipe, match?.missingCore)}>
    ...
    <div className="flex-1 min-w-0">
      ...
      <div className="text-xs text-stone-500 flex items-center gap-2 mt-0.5 whitespace-nowrap overflow-hidden">
        <span className="flex items-center gap-1 shrink-0"><Clock3 size={12} /> {recipe.time} min</span>
    ...
    <div className="flex items-center gap-2 shrink-0">
      ...
      <button className="card-heart" onClick={event => { event.stopPropagation(); onToggleFavorite(recipe.id) }} aria-label={isFavorite ? (lang === 'ms' ? 'Buang dari kegemaran' : 'Remove from favorites') : (lang === 'ms' ? 'Simpan resipi' : 'Save recipe')}><Heart size={17} fill={isFavorite ? 'currentColor' : 'none'} /></button>
